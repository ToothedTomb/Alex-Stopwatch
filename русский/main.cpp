...
static void on_start_button_clicked(GtkButton *button, gpointer user_data) {
    if (stopwatch_state == STOPWATCH_RESET) {
        ...
        gtk_button_set_label(GTK_BUTTON(start_button), "Пауза"); //The three stages start/ pause / contine then it goes back to pause
        ...
        gtk_button_set_label(GTK_BUTTON(start_button), "Продолжать");
    } else if (stopwatch_state == STOPWATCH_PAUSED) {
        g_timer_start(timer);
        stopwatch_state = STOPWATCH_RUNNING;
        gtk_button_set_label(GTK_BUTTON(start_button), "Пауза");
    }
}
...
static void on_reset_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    gtk_button_set_label(GTK_BUTTON(start_button), "Начинать"); //When the reset button is pressed it will reset the start button back to start to redo the step method.
}

static void activate(GtkApplication *app, gpointer user_data) {
    GtkWidget *window = gtk_application_window_new(app);
    gtk_window_set_title(GTK_WINDOW(window), "Алекс 2.0 - приложение секундомер");
    ...
    GtkWidget *title_label = gtk_label_new("Секундомер:");
    ...
    start_button = gtk_button_new_with_label("Начинать");
    gtk_widget_add_css_class(start_button, "button");
    gtk_box_append(GTK_BOX(vbox), start_button);

    GtkWidget *reset_button = gtk_button_new_with_label("Перезагрузить");
